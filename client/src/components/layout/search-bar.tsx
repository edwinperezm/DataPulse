        ...
        <div className="absolute top-full mt-2 right-0 w-64 max-h-96 overflow-y-auto bg-white rounded-lg shadow-lg border border-gray-200 z-50">
          <div className="py-2">
            {results.map((client) => (
              ...
              >
                <div className={`flex items-center justify-center w-8 h-8 rounded-full mr-2 text-xs font-semibold ${getStatusColor(client.status as ClientStatus).bg}`}>
                  {client.initials}
                </div>
                <div>
                  <div className="text-sm font-medium text-gray-900">{client.name}</div>
                  <div className="text-xs text-gray-500">{getStatusLabel(client.status as ClientStatus)}</div>
                </div>
              </button>
            ))}
          </div>
